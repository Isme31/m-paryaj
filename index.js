...
app.post('/admin/update-balance', async (req, res) => {
    ...
    // Fòse amount tounen chif pou evite erè 1000 -> 100
...
app.post('/request-withdraw', async (req, res) => {
    ...
    if (user && user.balance >= amt && amt >= 100) {
        ...
        const fee = amt * 0.05;
        await Withdraw.create({ phone: phone.trim(), amount: amt - fee, fee: fee, originalAmount: amt });
        res.json({ success: true, newBalance: user.balance - amt });
    } else { res.json({ success: false, msg: "Balans pa ase!" }); }
});
...
io.on('connection', (socket) => {
    socket.on('findMatch', async (data) => {
        ...
        if (!user || user.balance < bet) return socket.emit('gameOver', { msg: "Balans twò piti!" });
